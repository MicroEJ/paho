 ...
 * <p>
 * It provides applications a simple programming interface to all features of the MQTT version 3.1 specification
 * including:
 * <ul>
 * <li>connect
 * <li>publish
 ...
 * <ul>
 * <li>IMqttAsyncClient provides a set of non-blocking methods that return control to the invoking application after
 * initial validation of parameters and state. The main processing is performed in the background so as not to block the
 * application program's thread. This non- blocking approach is handy when the application needs to carry on processing
 * while the MQTT action takes place. For instance connecting to an MQTT server can take time, using the non-blocking
 * connect method allows an application to display a busy indicator while the connect action takes place in the
 * background. Non blocking methods are particularly useful in event oriented programs and graphical programs where
 * invoking methods that take time to complete on the the main or GUI thread can cause problems. The non-blocking
 * interface can also be used in blocking form.</li>
 * <li>IMqttClient provides a set of methods that block and return control to the application program once the MQTT
 * action has completed. It is a thin layer that sits on top of the IMqttAsyncClient implementation and is provided
 * mainly for compatibility with earlier versions of the MQTT client. In most circumstances it is recommended to use
 * IMqttAsyncClient based clients which allow an application to mix both non-blocking and blocking calls.</li>
 * </ul>
 * <p>
 * An application is not restricted to using one style if an IMqttAsyncClient based client is used as both blocking and
 * non-blocking methods can be used in the same application. If an IMqttClient based client is used then only blocking
 * methods are available to the application. For more details on the blocking client see {@link IMqttClient}
 * </p>
 *
 * <p>
 ...
 *     </code>
 * <p>
 * In this form the method returns a token that can be used to track the progress of the action (method). The method
 * provides a waitForCompletion() method that once invoked will block until the action completes. Once completed there
 * are method on the token that can be used to check if the action completed successfully or not. For example to wait
 * until a connect completes: <code>
 *      IMqttToken conToken;
 ...
 * </p>
 * <p>
 * To turn a method into a blocking invocation the following form can be used: <code>
 *     IMqttToken token;
 ...
 *     </code>
 * <p>
 * In this form a callback is registered with the method. The callback will be notified when the action succeeds or
 * fails. The callback is invoked on the thread managed by the MQTT client so it is important that processing is
 * minimised in the callback. If not the operation of the MQTT client will be inhibited. For example to be notified
 * (called back) when a connect completes: <code>
 *     	IMqttToken conToken;
 ...
 *			}
 *		  });
 *      </code> An optional context object can be passed into the method which will then be made available in the
 * callback. The context is stored by the MQTT client) in the token which is then returned to the invoker. The token is
 * provided to the callback methods where the context can then be accessed.
 * </p>
 * </li>
 * </ol>
 * <p>
 * To understand when the delivery of a message is complete either of the two methods above can be used to either wait
 * on or be notified when the publish completes. An alternative is to use the
 * {@link MqttCallback#deliveryComplete(IMqttDeliveryToken)} method which will also be notified when a message has been
 * delivered to the requested quality of service.
 * </p>
 *
 */
public interface IMqttAsyncClient {
	 ...
	 * @return token used to track and wait for the connect to complete. The token will be passed to the callback
	 *         methods if a callback is set.
	 ...
	 * @throws MqttException
	 *             for non security related problems
	 * @return token used to track and wait for the connect to complete. The token will be passed to any callback that
	 *         has been set.
	 * @see #connect(MqttConnectOptions, Object, IMqttActionListener)
	 */
	 ...
	 * </p>
	 *
	 * @return token used to track and wait for disconnect to complete. The token will be passed to any callback that
	 *         has been set.
	 * @throws MqttException
	 ...
	 * called from inside {@link MqttCallback} methods.
	 * </p>
	 *
	 ...
	 * @return token used to track and wait for disconnect to complete. The token will be passed to the callback methods
	 ...
	 * @return token used to track and wait for the disconnect to complete. The token will be passed to any callback
	 *         that has been set.
	 * @throws MqttException
	 *             for problems encountered while disconnecting
	 * @see #disconnect(long, Object, IMqttActionListener)
	 ...
	 * Disconnects from the server.
	 * <p>
	 * The client will wait for {@link MqttCallback} methods to complete. It will then wait for up to the quiesce
	 ...
	 * <p>
	 * A convenience method, which will create a new {@link MqttMessage} object with a byte array payload and the
	 ...
	 * @return token used to track and wait for the publish to complete. The token will be passed to any callback that
	 *         has been set.
	...
	/**
	 * Publishes a message to a topic on the server. Takes an {@link MqttMessage} message and delivers it to the server
	 ...
	 * Publishes a message to a topic on the server.
	 * <p>
	 * Once this method has returned cleanly, the message has been accepted for publication by the client and will be
	 * delivered on a background thread. In the event the connection fails or the client stops. Messages will be
	 * delivered to the requested quality of service once the connection is re-established to the server on condition
	 * that:
	 * <ul>
	 ...
	 *
	 * <p>
	 * When building an application, the design of the topic tree should take into account the following principles of
	 * topic name syntax and semantics:
	 * </p>
	 *
	 * <ul>
	 * <li>A topic must be at least one character long.</li>
	 * <li>Topic names are case sensitive. For example, <em>ACCOUNTS</em> and <em>Accounts</em> are two different
	 * topics.</li>
	 * <li>Topic names can include the space character. For example, <em>Accounts
	 * 	payable</em> is a valid topic.</li>
	 * <li>A leading "/" creates a distinct topic. For example, <em>/finance</em> is different from <em>finance</em>.
	 * <em>/finance</em> matches "+/+" and "/+", but not "+".</li>
	 ...
	 *
	 * <ul>
	 * <li>The length is limited to 64k but within that there are no limits to the number of levels in a topic tree.
	 * </li>
	 ...
	 * {@link MqttCallback#deliveryComplete(IMqttDeliveryToken)} method will be called.</li>
	 ...
	 *            of service
	 * @return token used to track and wait for the publish to complete. The token will be passed to callback methods if
	 *         set.
	 * @throws MqttPersistenceException
	 *             when a problem occurs storing the message
	 * @throws IllegalArgumentException
	 ...
	 * @return token used to track and wait for the subscribe to complete. The token will be passed to callback methods
	 *         if set.
	 * @throws MqttException
	 ...
	 * </p>
	 * <p>
	 * The {@link #setCallback(MqttCallback)} method should be called before this method, otherwise any received
	 * messages will be discarded.
	 * </p>
	 * <p>
	 * If (@link MqttConnectOptions#setCleanSession(boolean)} was set to true when when connecting to the server then
	 * the subscription remains in place until either:
	 * <ul>
	 * <li>The client disconnects</li>
	 * <li>An unsubscribe method is called to un-subscribe the topic</li>
	 * </ul>
	 *
	 * If (@link MqttConnectOptions#setCleanSession(boolean)} was set to false when connecting to the server then the
	 * subscription remains in place until either:
	 * <ul>
	 ...
	 * </ul>
	 * With cleanSession set to false the MQTT server will store messages on behalf of the client when the client is not
	 * connected. The next time the client connects with the <strong>same client ID</strong> the server will deliver the
	 * stored messages to the client.
	 *
	 * <p>
	 * The "topic filter" string used when subscribing may contain special characters, which allow you to subscribe to
	 * multiple topics at once.
	 * </p>
	 * <p>
	 * The topic level separator is used to introduce structure into the topic, and can therefore be specified within
	 * the topic for that purpose. The multi-level wildcard and single-level wildcard can be used for subscriptions, but
	 * they cannot be used within a topic by the publisher of a message.
	 * <dl>
	 * <dt>Topic level separator</dt>
	 * <dd>The forward slash (/) is used to separate each level within a topic tree and provide a hierarchical structure
	 * to the topic space. The use of the topic level separator is significant when the two wildcard characters are
	 ...
	 * subscribe to <span><span class="filepath">finance/stock/ibm/#</span></span>, you receive messages on these
	 * topics:
	 *
	 ...
	 * <p>
	 * The multi-level wildcard can represent zero or more levels. Therefore, <em>finance/#</em> can also match the
	 * singular <em>finance</em>, where <em>#</em> represents zero levels. The topic level separator is meaningless in
	 * this context, because there are no levels to separate.
	 * </p>
	 *
	 * <p>
	 * The <span>multi-level</span> wildcard can be specified only on its own or next to the topic level separator
	 * character. Therefore, <em>#</em> and <em>finance/#</em> are both valid, but <em>finance#</em> is not valid.
	 * <span>The multi-level wildcard must be the last character used within the topic tree. For example,
	 * <em>finance/#</em> is valid but <em>finance/#/closingprice</em> is not valid.</span>
	 * </p>
	 * </dd>
	 *
	 ...
	 * <em>finance/stock/+</em> matches <em>finance/stock/ibm</em> and <em>finance/stock/xyz</em>, but not
	 * <em>finance/stock/ibm/closingprice</em>. Also, because the single-level wildcard matches only a single level,
	 * <em>finance/+</em> does not match <em>finance</em>.
	 * </p>
	 *
	 * <p>
	 * Use the single-level wildcard at any level in the topic tree, and in conjunction with the multilevel wildcard.
	 * Specify the single-level wildcard next to the topic level separator, except when it is specified on its own.
	 * Therefore, <em>+</em> and <em>finance/+</em> are both valid, but <em>finance+</em> is not valid. <span>The
	 * single-level wildcard can be used at the end of the topic tree or within the topic tree. For example,
	 ...
	 * @return token used to track and wait for the unsubscribe to complete. The token will be passed to callback
	 *         methods if set.
	 * @throws MqttException
	 ...
	 * Requests the server unsubscribe the client from one or more topics.
	 * <p>
	 * Unsubcribing is the opposite of subscribing. When the server receives the unsubscribe request it looks to see if
	 * it can find a matching subscription for the client and then removes it. After this point the server will send no
	 * more messages to the client for this subscription.
	 * </p>
	 * <p>
	 * The topic(s) specified on the unsubscribe must match the topic(s) specified in the original subscribe request for
	 * the unsubscribe to succeed
	 * </p>
	 * <p>
	 ...
	 * using the {@link MqttToken} returned from each non-blocking method or using setting a {@link IMqttActionListener}
	 ...
	 * the {@link MqttCallback#deliveryComplete(IMqttDeliveryToken)} callback can be used to track the delivery of
	 * outstanding messages.
	 * </p>
	 * <p>
	 * If a client connects with cleanSession true then there will be no delivery tokens as the cleanSession option
	 * deletes all earlier state. For state to be remembered the client must connect with cleanSession set to false
	 * </P>
	 *
