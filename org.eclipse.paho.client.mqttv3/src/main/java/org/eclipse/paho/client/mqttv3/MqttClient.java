...
public class MqttClient implements IMqttClient { // ), DestinationProvider {
	 ...
	 * </ul>
	 * The default value is -1 which means the action will not timeout. In the event of a timeout the action carries on
	 * running in the background until it completes. The timeout is used on methods that block while the action is in
	 * progress.
	 *
