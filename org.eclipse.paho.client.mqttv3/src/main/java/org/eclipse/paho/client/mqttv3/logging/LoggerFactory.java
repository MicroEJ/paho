 ...
 *
 * The default log and trace facility uses Java's build in log facility:- java.util.logging. For systems where this is
 * not available or where an alternative logging framework is required the logging facility can be replaced using
 * {@link org.eclipse.paho.client.mqttv3.logging.LoggerFactory#setLogger(String)} which takes an implementation of the
...
public class LoggerFactory {
	...
	/**
	 * Find or create a logger for a named package/class. If a logger has already been created with the given name it is
	...
	private static void loadInternalLoggerFactoryIfNull() {
		if (internalLoggerFactory == null) {
			internalLoggerFactory = (InternalLoggerFactory) DependencyInjectionHelper
					.getImplementation(InternalLoggerFactory.class);
		}
	}

	/**
	 * When run in JSR47, this allows access to the properties in the logging.properties file. If not run in JSR47, or
	 * the property isn't set, returns null.
	 *
	 * @param name
