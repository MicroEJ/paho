 ...
 *
 * 1) When the client is not running messages are stored in a persistent store that implements the MqttClientPersistent
 * Interface. The default is MqttDefaultFilePersistencew which stores messages safely across failures and system
 * restarts. If no persistence is specified there is a fall back to MemoryPersistence which will maintain the messages
 * while the Mqtt client is instantiated.
 *
 * 2) When the client or specifically ClientState is instantiated the messages are read from the persistent store into:
 * - outboundqos2 hashtable if a QoS 2 PUBLISH or PUBREL - outboundqos1 hashtable if a QoS 1 PUBLISH (see restoreState)
 *
 * 3) On Connect, copy messages from the outbound hashtables to the pendingMessages or pendingFlows vector in messageid
 * order. - Initial message publish goes onto the pendingmessages buffer. - PUBREL goes onto the pendingflows buffer
 * (see restoreInflightMessages)
 *
 * 4) Sender thread reads messages from the pendingflows and pendingmessages buffer one at a time. The message is
 * removed from the pendingbuffer but remains on the outbound* hashtable. The hashtable is the place where the full set
 * of outstanding messages are stored in memory. (Persistence is only used at start up)
 *
 * 5) Receiver thread - receives wire messages: - if QoS 1 then remove from persistence and outboundqos1 - if QoS 2
 * PUBREC send PUBREL. Updating the outboundqos2 entry with the PUBREL and update persistence. - if QoS 2 PUBCOMP remove
 * from persistence and outboundqos2
 *
 * Notes: because of the multithreaded nature of the client it is vital that any changes to this class take concurrency
 * into account. For instance as soon as a flow / message is put on the wire it is possible for the receiving thread to
 * receive the ack and to be processing the response before the sending side has finished processing. For instance a
...
public class ClientState {
	...
	private final int maxInflight = DEFAULT_MAX_INFLIGHT;
	...
	/**
	 * Inserts a new message to the list, ensuring that list is ordered from lowest to highest in terms of the message
	 * id's.
	 *
	 * @param list
	 ...
	 *
	 * @param list
	 *            the list containing the messages to produce a new reordered list for - this will not be modified or
	 *            replaced, i.e., be read-only to this method
	...
	protected void restoreState() throws MqttException {
		...
		while (messageKeys.hasMoreElements()) {
			...
			persistable = this.persistence.get(key);
			...
			if (message != null) {
				if (key.startsWith(PERSISTENCE_RECEIVED_PREFIX)) {
					...
					if (this.persistence.containsKey(getSendConfirmPersistenceKey(sendMessage))) {
						MqttPersistable persistedConfirm = this.persistence
								.get(getSendConfirmPersistenceKey(sendMessage));
					...
					this.inUseMsgIds.put(new Integer(sendMessage.getMessageId()),
							new Integer(sendMessage.getMessageId()));
	...
	/**
	 * Submits a message for delivery. This method will block until there is room in the inFlightWindow for the message.
	 * The message is put into persistence before returning.
	 *
	 * @param message
	 *            the message to send
	 * @param token
		...
		if (message instanceof MqttPublish) {
			synchronized (this.queueLock) {
				if (this.actualInFlight >= this.maxInflight) {
					...
					log.fine(CLASS_NAME, methodName, "613", new Object[] { new Integer(this.actualInFlight) });
	...
	/**
	 * Check and send a ping if needed and check for ping timeout. Need to send a ping if nothing has been sent or
	 * received in the last keepalive interval. It is important to check for both sent and received packets in order to
	 * catch the case where an app is solely sending QoS 0 messages or receiving QoS 0 messages. QoS 0 message are not
	 * good enough for checking a connection is alive as they are one way messages.
	 *
	 * If a ping has been sent but no data has been received in the last keepalive interval then the connection is
	 * deamed to be broken.
	 *
	...
	public MqttToken checkForActivity() throws MqttException {
		...
		if (this.connected && this.keepAlive > 0) {
			...
			synchronized (this.pingOutstandingLock) {
				...
				if (this.pingOutstanding > 0 && (time - this.lastInboundActivity >= this.keepAlive + delta)) {
					...
					log.severe(CLASS_NAME, methodName, "619",
							new Object[] { new Long(this.keepAlive), new Long(this.lastOutboundActivity),
									new Long(this.lastInboundActivity), new Long(time), new Long(this.lastPing) });
				...
				if (this.pingOutstanding == 0 && (time - this.lastOutboundActivity >= 2 * this.keepAlive)) {
					...
					// I am probably blocked on a write operations as I should have been able to write at least a ping
					// message
				...
				// In this case only one ping is sent. If not confirmed, client will assume a lost connection to the
				// broker.
				...
				// the broker needs to keep receiving a regular ping even if the ping response are queued after the long
				// message
				// If lacking to do so, the broker will consider my connection lost and cut my socket.
				if ((this.pingOutstanding == 0 && (time - this.lastInboundActivity >= this.keepAlive - delta))
						|| (time - this.lastOutboundActivity >= this.keepAlive - delta)) {
					...
					log.fine(CLASS_NAME, methodName, "620", new Object[] { new Long(this.keepAlive),
							new Long(this.lastOutboundActivity), new Long(this.lastInboundActivity) });
	...
	/**
	 * This returns the next piece of work, ie message, for the CommsSender to send over the network. Calls to this
	 * method block until either: - there is a message to be sent - the keepAlive interval is exceeded, which triggers a
	 * ping message to be returned - {@link #disconnected(MqttException)} is called
	 *
		...
		synchronized (this.queueLock) {
			while (result == null) {
				...
				if ((this.pendingMessages.isEmpty() && this.pendingFlows.isEmpty())
						|| (this.pendingFlows.isEmpty() && this.actualInFlight >= this.maxInflight)) {
					try {
				...
				if (!this.connected && (this.pendingFlows.isEmpty()
						|| !((MqttWireMessage) this.pendingFlows.elementAt(0) instanceof MqttConnect))) {
				...
				if (!this.pendingFlows.isEmpty()) {
					...
					if (result instanceof MqttPubRel) {
						...
						log.fine(CLASS_NAME, methodName, "617", new Object[] { new Integer(this.inFlightPubRels) });
					}
					...
					if (this.actualInFlight < this.maxInflight) {
						...
						log.fine(CLASS_NAME, methodName, "623", new Object[] { new Integer(this.actualInFlight) });
					} else {
	...
	/**
	 * COMMENTED OUT AS NO LONGER USED. Deduce how long to to wait until a ping is required.
	 *
	 * In order to keep the connection alive the server must see activity within the keepalive interval. If the
	 * application is not sending / receiving any messages then the client will send a ping. This method works out the
	 * next time that a ping must be sent in order for the server to know the client is alive.
	 *
	 * The time before a ping needs to be sent to keep alive the connection long getTimeUntilPing() { long pingin =
	 * getKeepAlive(); // If KA is zero which means just wait for work or // if a ping is outstanding return the KA
	 * value if (connected &amp;&amp; (getKeepAlive() &gt; 0) &amp;&amp; !pingOutstanding) {
	 *
	 * long time = System.currentTimeMillis(); long timeSinceOut = (time-lastOutboundActivity); long timeSinceIn =
	 * (time-lastInboundActivity);
	 *
	 * if (timeSinceOut &gt; timeSinceIn) { pingin = (getKeepAlive()-timeSinceOut); } else { pingin =
	 * (getKeepAlive()-timeSinceIn); }
	 *
	 * // Unlikely to be negative or zero but in the case it is return a // small value &gt; 0 to cause a ping to occur
	 * if (pingin &lt;= 0) { pingin = 10; } } return (pingin); }
	 */
	...
	protected void notifySent(MqttWireMessage message) {
		...
		if (message instanceof MqttPingReq) {
			synchronized (this.pingOutstandingLock) {
				...
				log.fine(CLASS_NAME, methodName, "635", new Object[] { new Integer(this.pingOutstanding) });
			}
	...
	private void decrementInFlight() {
		...
		synchronized (this.queueLock) {
			...
			log.fine(CLASS_NAME, methodName, "646", new Object[] { new Integer(this.actualInFlight) });
	...
	protected boolean checkQuiesceLock() {
		...
		if (this.quiescing && tokC == 0 && this.pendingFlows.size() == 0 && this.callback.isQuiesced()) {
			// @TRACE 626=quiescing={0} actualInFlight={1} pendingFlows={2} inFlightPubRels={3} callbackQuiesce={4}
			// tokens={5}
			log.fine(CLASS_NAME, methodName, "626",
					new Object[] { new Boolean(this.quiescing), new Integer(this.actualInFlight),
							new Integer(this.pendingFlows.size()), new Integer(this.inFlightPubRels),
							Boolean.valueOf(this.callback.isQuiesced()), new Integer(tokC) });
			synchronized (this.quiesceLock) {
	...
	protected void notifyReceivedAck(MqttAck ack) throws MqttException {
		...
		if (ack instanceof MqttPubRec) {
			...
			log.fine(CLASS_NAME, methodName, "636", new Object[] { new Integer(this.pingOutstanding) });
	...
	/**
	 * Called by the CommsReceiver when a message has been received. Handles inbound messages and other flows such as
	 * PUBREL.
	 *
	 * @param message
	 * @throws MqttException
	...
	/**
	 * Called when waiters and callbacks have processed the message. For messages where delivery is complete the message
	 * can be removed from persistence and counters adjusted accordingly. Also tidy up by removing token from store...
	 *
	 * @param token
	 * @throws MqttException
	 */
		...
		if (message != null && message instanceof MqttAck) {
			...
			if (ack instanceof MqttPubAck) {
				...
				log.fine(CLASS_NAME, methodName, "645",
						new Object[] { new Integer(ack.getMessageId()), new Integer(this.inFlightPubRels) });
			}
	...
	public void connected() {
		...
		this.pingSender.start(); // Start ping thread when client connected to server.
	}

	/**
	 * Called during shutdown to work out if there are any tokens still to be notified and waiters to be unblocked.
	 * Notifying and unblocking takes place after most shutdown processing has completed. The tokenstore is tidied up so
	 * it only contains outstanding delivery tokens which are valid after reconnect (if clean session is false)
	 *
	 * @param reason
	...
	/**
	 * Releases a message ID back into the pool of available message IDs. If the supplied message ID is not in use, then
	 * nothing will happen.
	 *
	 * @param msgId
	...
	/**
	 * Quiesce the client state, preventing any new messages getting sent, and preventing the callback on any newly
	 * received messages. After the timeout expires, delete any pending messages except for outbound ACKs, and wait for
	 * those ACKs to complete.
	 */
			...
			synchronized (this.quiesceLock) {
				try {
					...
					if (tokc > 0 || this.pendingFlows.size() > 0 || !this.callback.isQuiesced()) {
						// @TRACE 639=wait for outstanding: actualInFlight={0} pendingFlows={1} inFlightPubRels={2}
						// tokens={3}
						log.fine(CLASS_NAME, methodName, "639",
								new Object[] { new Integer(this.actualInFlight), new Integer(this.pendingFlows.size()),
										new Integer(this.inFlightPubRels), new Integer(tokc) });
