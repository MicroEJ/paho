...
public class MqttMessage {
	...
	/**
	 * Whether or not the publish message should be retained by the messaging engine. Sending a message with the
	 * retained set to <code>false</code> will clear the retained message from the server. The default value is
	 * <code>false</code>
	 *
	 * @param retained
	 ...
	 * but should only be used for messages which are not valuable - note that if the server cannot process the message
	 * (for example, there is an authorization problem), then an
	 * {@link MqttCallback#deliveryComplete(IMqttDeliveryToken)}. Also known as "fire and forget".</li>
	 *
	 * <li>Quality of Service 1 - indicates that a message should be delivered at least once (one or more times). The
	 * message can only be delivered safely if it can be persisted, so the application must supply a means of
	 * persistence using <code>MqttConnectOptions</code>. If a persistence mechanism is not specified, the message will
	 * not be delivered in the event of a client failure. The message will be acknowledged across the network. This is
	 * the default QoS.</li>
	 *
	 * <li>Quality of Service 2 - indicates that a message should be delivered once. The message will be persisted to
	 * disk, and will be subject to a two-phase acknowledgement across the network. The message can only be delivered
	 * safely if it can be persisted, so the application must supply a means of persistence using
	 * <code>MqttConnectOptions</code>. If a persistence mechanism is not specified, the message will not be delivered
	 * in the event of a client failure.</li>
	 * </ul>
	 *
	 * If persistence is not configured, QoS 1 and 2 messages will still be delivered in the event of a network or
	 * server problem as the client will hold state in memory. If the MQTT client is shutdown or fails and persistence
	 * is not configured then delivery of QoS 1 and 2 messages can not be maintained as client-side state will be lost.
	 *
	 * @param qos
	...
	/**
	 * Returns whether or not this message might be a duplicate of one which has already been received. This will only
	 * be set on messages received from the server.
	 *
