...
import org.eclipse.paho.client.mqttv3.internal.NetworkModuleFactory;
 ...
 * </ul>
 * <p>
 * To enable messages to be delivered even across network and client restarts messages need to be safely stored until
 * the message has been delivered at the requested quality of service. A pluggable persistence mechanism is provided to
 * store the messages.
 * </p>
 * <p>
 * If connecting with {@link MqttConnectOptions#setCleanSession(boolean)} set to true it is safe to use memory
 ...
 * {@link MqttClientPersistence} interface and passing it to the clients constructor.
 * </p>
 *
...
public class MqttAsyncClient implements IMqttAsyncClient { // DestinationProvider
	 ...
	 * can be specified using the {@link MqttConnectOptions#setServerURIs(String[]) setServerURIs} method on
	 * MqttConnectOptions.
	 *
	 * <p>
	 ...
	 * {@link #generateClientId()}. As the client identifier is used by the server to identify a client when it
	 ...
	 * </p>
	 * <ul>
	 * <li><strong>Supplying an <code>SSLSocketFactory</code></strong> - applications can use
	 * {@link MqttConnectOptions#setSocketFactory(SocketFactory)} to supply a factory with the appropriate SSL settings.
	 * </li>
	 * <li><strong>SSL Properties</strong> - applications can supply SSL settings as a simple Java Properties using
	 * {@link MqttConnectOptions#setSSLProperties(Properties)}.</li>
	 * <li><strong>Use JVM settings</strong> - There are a number of standard Java system properties that can be used to
	 * configure key and trust stores.</li>
	 * </ul>
	 *
	 * <p>
	 ...
	 * {@link MqttClientPersistence} interface. An implementer of this interface that safely stores messages must be
	 * specified in order for delivery of messages to be reliable. In addition
	 * {@link MqttConnectOptions#setCleanSession(boolean)} must be set to false. In the event that only QoS 0 messages
	...
	public MqttAsyncClient(String serverURI, String clientId, MqttClientPersistence persistence,
		...
		for (int i = 0; i < clientId.length() - 1; i++) {
			if (Strings.Character_isHighSurrogate(clientId.charAt(i))) {
				i++;
			}
			clientIdLength++;
		}
		...
		if (this.persistence == null) {
			this.persistence = (MqttClientPersistence) DependencyInjectionHelper
					.getImplementation(MqttClientPersistence.class);
		}
	...
	private NetworkModule createNetworkModule(String address, MqttConnectOptions options)
		...
		NetworkModuleFactory networkModuleFactory = (NetworkModuleFactory) DependencyInjectionHelper
				.getImplementation(NetworkModuleFactory.class);
		return networkModuleFactory.createNetworkModule(address, options, this.clientId);
	}

	/*
	 * (non-Javadoc)
	 *
	 * @see org.eclipse.paho.client.mqttv3.IMqttAsyncClient#connect(java.lang.Object,
	...
	public IMqttToken connect(MqttConnectOptions options, Object userContext, IMqttActionListener callback)
		...
		ConnectActionListener connectActionListener = new ConnectActionListener(this, this.persistence, this.comms,
				options, userToken, userContext, callback);
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.eclipse.paho.client.mqttv3.IMqttAsyncClient#disconnect(java.lang. Object,
	 * org.eclipse.paho.client.mqttv3.IMqttActionListener)
	 */
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.eclipse.paho.client.mqttv3.IMqttAsyncClient#disconnect(long, java.lang.Object,
	 * org.eclipse.paho.client.mqttv3.IMqttActionListener)
	 */
	 ...
	 * <ul>
	 * <li>{@link MqttAsyncClient#publish(java.lang.String, byte[], int, boolean)} to publish a message in a
	 * non-blocking manner or
	 * <li>{@link MqttClient#publish(java.lang.String, org.eclipse.paho.client.mqttv3.MqttMessage)} to publish a message
	 * in a blocking manner
	 * </ul>
	 * <p>
	 * When you build an application, the design of the topic tree should take into account the following principles of
	...
	/*
	 * (non-Javadoc) Check and send a ping if needed. <p>By default, client sends PingReq to server to keep the
	 * connection to server. For some platforms which cannot use this mechanism, such as Android, developer needs to
	 * handle the ping request manually with this method. </p>
	 *
	...
	public IMqttToken checkPing(Object userContext, IMqttActionListener callback) throws MqttException {
		...
		token = this.comms.checkForActivity();
		// @TRACE 118=<
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.eclipse.paho.client.mqttv3.IMqttAsyncClient#subscribe(java.lang.String , int, java.lang.Object,
	 * org.eclipse.paho.client.mqttv3.IMqttActionListener)
	 */
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.eclipse.paho.client.mqttv3.IMqttAsyncClient#unsubscribe(java.lang .String, java.lang.Object,
	 * org.eclipse.paho.client.mqttv3.IMqttActionListener)
	 */
	 ...
	 * <p>
	 * When cleanSession is set to false, an application must ensure it uses the same client identifier when it
	 * reconnects to the server to resume state and maintain assured message delivery.
	 * </p>
	 *
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.eclipse.paho.client.mqttv3.IMqttAsyncClient#publish(java.lang.String,
