 ...
 * application programs thread. This non blocking approach is handy when the application wants to carry on processing
 * while the MQTT action takes place. For instance connecting to an MQTT server can take time, using the non-blocking
 * connect method allows an application to display a busy indicator while the connect action is occurring. Non-blocking
 * methods are particularly useful in event-oriented programs and graphical programs where issuing methods that take
 * time to complete on the the main or GUI thread can cause problems.</li>
 * </ul>
 * <p>
 * The non-blocking client can also be used in a blocking form by turning a non-blocking method into a blocking
 * invocation using the following pattern: <code>
 *     IMqttToken token;
 *     token = asyncClient.method(parms).waitForCompletion();
 *     </code> Using the non-blocking client allows an application to use a mixture of blocking and non-blocking styles.
 * Using the blocking client only allows an application to use one style. The blocking client provides compatibility
 * with earlier versions of the MQTT client.
 * </p>
 */
public interface IMqttClient { // extends IMqttAsyncClient {
	 ...
	 * Disconnects from the server.
	 * <p>
	 * The client will wait for all {@link MqttCallback} methods to complete. It will then wait for up to the quiesce
	 ...
	 * </ul>
	 * <p>
	 * If (@link MqttConnectOptions#setCleanSession(boolean)} was set to false when when connecting to the server then
	 * the subscription remains in place until either:
	 * <ul>
	 ...
	 * it can find a subscription for the client and then removes it. After this point the server will send no more
	 ...
	 * been delivered. In the event the connection fails or the client stops, any messages that are in the process of
	 * being delivered will be delivered once a connection is re-established to the server on condition that:
	 * <ul>
	 ...
	 * </ul>
	 * <p>
	 * In the event that the connection breaks or the client stops it is still possible to determine when the delivery
	 * of the message completes. Prior to re-establishing the connection to the server:
	 * <ul>
	 * <li>Register a {@link #setCallback(MqttCallback)} callback on the client and the delivery complete callback will
	 * be notified once a delivery of a message completes
	 * <li>or call {@link #getPendingDeliveryTokens()} which will return a token for each message that is in-flight. The
	 ...
	 * using the {@link MqttToken} passed to the operation
	 * <p>
	 *
	 * @see MqttCallback
	 * @param callback
	 ...
	 * {@link MqttCallback#deliveryComplete(IMqttDeliveryToken)} callback can be used to track the delivery of
