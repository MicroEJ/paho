...
public class MqttConnectOptions {
	...
	/**
	 * Returns the message to be sent as last will and testament (LWT). The returned object is "read only". Calling any
	 * "setter" methods on the returned object will result in an <code>IllegalStateException</code> being thrown.
	 *
	...
	/**
	 * Sets the SSL properties for the connection. Note that these properties are only valid if an implementation of the
	 * Java Secure Socket Extensions (JSSE) is available. These properties are <em>not</em> used if a SocketFactory has
	 * been set using {@link #setSocketFactory(SocketFactory)}. The following properties can be used:
	 * <dl>
